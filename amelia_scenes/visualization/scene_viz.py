...
def plot_scene(
    ...
    if not len(features_to_add):
        sequences = scenario['sequences'][:, :, G.H]

        plot_scene_simple(scenario, assets, filetag, dpi=dpi)
    else:
...
def plot_scene_simple(
    ...
    # Display global map
    movement_plot.imshow(bkg, zorder=0, extent=[west, east, south, north], alpha=.1) 
    movement_plot.set_facecolor('slategrey')
    
    # Iterate through agent in the scene
    all_lon, all_lat, scores = [], [], []

    for n, (trajectory, agent_type) in enumerate(zip(trajectories, agent_types)):
    ...
    movement_plot.spines['top'].set_visible(False)
    movement_plot.spines['bottom'].set_visible(False)
    movement_plot.spines['left'].set_visible(False)
    movement_plot.spines['right'].set_visible(False)
    
    # Set figure bbox around the predicted trajectory
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0) 
    plt.savefig(f'{out_dir}/{tag}.png', dpi=400, bbox_inches='tight', pad_inches=0)
    plt.close()
