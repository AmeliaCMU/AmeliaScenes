...
def plot_scene_benchmark_preds(
    ...
    hist_color, hist_lw = C.MOTION_COLORS['gt_hist'][0], C.MOTION_COLORS['gt_hist'][1]
    fut_color, fut_lw = C.MOTION_COLORS['gt_hist_missing'][0], C.MOTION_COLORS['gt_hist_missing'][1]
    ...
    for n, (gt_hist, gt_fut, agent_type, mask, agent_id) in enumerate(zipped):
        ...
        if not gt_fut.shape[0] == 0:
            gt_fut_ll = C.reproject_sequences(
                gt_fut[:, 1:], projection) if reproject else gt_fut[:, 1:]
