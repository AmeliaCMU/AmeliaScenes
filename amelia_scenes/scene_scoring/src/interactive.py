...
def compute_interactive_metrics(
    ...
    hold_lines: np.array, 
    return_critical_value: bool = True,       # Returns most critical value from the interactions
    stationary_speed_thresh: float = 10.0,    # Speed (Km/H) to consider an agent as stationary
    agent_to_agent_dist_thresh: float = 1.0,  # Airport-specific: airplanning.com/post/airport-runways
    closest_point_dist_thresh: float = 0.1,   # [Arbitrary] 100m distance to a hold-line
    separation_dist_thresh: float = 0.300,    # Separation standards: airservicesaustralia.com
) -> dict:
