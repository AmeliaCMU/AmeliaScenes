...
if __name__ == "__main__":
    ...
    scenarios_subdirs = [os.path.join(args.base_dir, "proc_trajectories", f) for f in subdirs]
    ...
    singles_anomaly = np.load(singles_filepath, allow_pickle=True)['arr_0'].item()
    ...
    for n, scenario in enumerate(tqdm(scenarios)):
        ...
        metrics = M.compute_individual_metrics(sequences=scene['sequences'], hold_lines=hl_xy)
        ...
        if args.compute_scores:
            ...
            scores, scene_score = S.compute_individual_scores(metrics, agent_types)
            ...
            if args.plot:
                ...
                filetag = os.path.join(outdir, f"{subdir}_{scenario_id}_score-{round(scene_score, 2)}")
    ...
    for k, v in combined_metrics.items():
        ...
        plt.title(f"Airport: {airport.upper()} Metric: {k} Split: {args.split}")
