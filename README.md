...
**AmeliaScenes**: Tool for generating airport surface movement scenes from raw trajectory data collected with [AmeliaSWIM](https://github.com/AmeliaCMU/AmeliaSWIM). 
...
Once downloaded, create a symbolic link in `datasets` from the AmeliaScenes repository:
        ...
        |-- amelia
            |-- traj_data_a10v08
                |-- raw_trajectories
                    ...
                    |-- panc
```
For more details about the dataset, see [here](https://ameliacmu.github.io/amelia-dataset/).
...
Or, refer to [`INSTALL.md`](https://github.com/AmeliaCMU/AmeliaScenes/blob/main/INSTALL.md) for manual installation.
...
Once the tools and environment are set up, process the data and generate the '.pkl' files:
...
This script will process all the raw trajectory CSV files found in `datasets/amelia/traj_data_a10v08/raw_trajectories/<airport_icao>/` and generate scenes for each of the airports `kbos`, `kdca`, `kewr`, `kjfk`, `klax`, `kmdw`, `kmsy`, `ksea`, `ksfo`, and `panc` [ICAO](https://en.wikipedia.org/wiki/ICAO_airport_code).
...
Once the scenes are generated, run `run_create_splits.py` to split the dataset. The script can be run as follows:
...
- `<base_dir>`: Path to the dataset. By default, the path is set to `../datasets/amelia`.
- `<traj_version>`: Version of the trajectory data. By defaul,t it is set to `a10v08` to match the current released version.
- `<seed>`: Seed for the random number generator. By default, it is set to `42`.
